...
int Open3916(int argc, char ** argv, fitsfile ** input, fitsfile ** output, char * infileTCSP, char * infileOBTUTC) {
	...
	fits_select_rows(output[0], output[0], "TMTYPE == 33 && TMSTYPE==1 && TCTYPE==33 && (TCSTYPE==23)", &status);

	long nrows = 0;
	fits_get_num_rows(output[0], &nrows, &status);


	if (nrows == 0) {
		fits_movabs_hdu(input[0], 2, NULL, &status);
		fits_select_rows(input[0], output[0], "TMTYPE == 33 && TMSTYPE==1 && TCTYPE==35 && (TCSTYPE==1)", &status);	
		}
...
///////////////////////////////////////////////////////////////////////////////////////////////	
	...
	int tctype = 0, tcstype = 0;
	fits_get_colnum(output, 1, "TCTYPE", &colnum, &status);	
	fits_read_col(output, TINT32BIT, colnum,  1, 1, 1, NULL, &tctype, NULL, &status);	
...
// 	fits_select_rows(output, output, "TIME > 110553540.000", &status);	
	...
	if (nrows == 0) {
		cout<<"NON CI SONO TC ENTER IDLE "<<nrows<<" "<<status<<endl;

		}
	else {
		cout<<"Numero Eventi = "<<nrows<<endl;
		}
...
// 	fits_get_num_cols(output, &ncols, &status);	
	...
	char expr[1024];
	char expr2[1024];	
	sprintf(expr, "TIME >= %f && TIME <= %f && TCTYPE==33 && (TCSTYPE==23)", tstart-2000, tstop+2000);	

	if (tctype == 35)
		sprintf(expr, "TIME >= %f && TIME <= %f && TCTYPE==35 && (TCSTYPE==1)", tstart-2000, tstop+2000);	

	sprintf(expr2, "TIME >= %f && TIME <= %f", tstart-20000, tstop+20000);


 	status = addfileTCSP(&inputTCSP[0], infileTCSP, expr, tstart-20000, tstop+2000);
	if (status == 1005) {
	...
	if (status == 1005) {
		printf("ERROR!! OBTUTC DOESN'T FOUND\n\n\n");
	printf("\n\n####################################################\n");
	printf("#########  Task Cor33XY_bis......... exiting #########\n");
	printf("####################################################\n\n\n");
		return status;
...
// 		printf("ERRORE   TEMPO REALE - OBDH = %f\n",timediffobhreal);	
		int tctype = 0, tcstype = 0;
		fits_get_colnum(output, 1, "TCTYPE", &colnum, &status);	
		fits_read_col(output, TINT32BIT, colnum,  i+1, 1, 1, NULL, &tctype, NULL, &status);
		fits_get_colnum(output, 1, "TCSTYPE", &colnum, &status);	
		fits_read_col(output, TINT32BIT, colnum,  i+1, 1, 1, NULL, &tcstype, NULL, &status);

		printf("TCTYPE=%d TCSTYPE=%d\n",tctype,tcstype);		
		printf("(PDHUtime+DelayTimeOBDH) - RealTimeTC -DelayTCsent = %f\n",(time[i]+timediffobhreal)-timeTCSP[it]-0.159);				
	...
	cout<<"Eventi Trovati = "<<numeribuoni<<endl;	
	
	if (numeribuoni==0) {

		cout<<"Non è stato possibile calcolare il Drift = "<<numeribuoni<<endl;	
